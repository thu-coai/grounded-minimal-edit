...
from dataloaders.blendedskilltalk import nltk_convai_normalize
import random
import json
...
class BertEval(object):
    ...
    def read_intervening_persona(self, data_mode):
        ...
        # Process bst data
        bst_name = os.path.join('../data', 'blendedskilltalk', '{}.txt'.format(data_mode))
        print(("Reading lines from {}".format(bst_name)))
        intervening_persona = []
        with open(bst_name) as fbst:
            for line in fbst.readlines():
                ...
                intervening_persona.append(personas)
