...
class Attention(nn.Module):
    ...
    def forward(self, x, layer_past=None, attention_mask=None, head_mask=None):
        ...
        present = torch.stack((key.transpose(-2, -1), value), dim=1)  # transpose to have same shapes for stacking
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
        ...
        **past**:
            list of ``torch.FloatTensor`` (one for each layer) of shape ``(batch_size, 2, num_heads, sequence_length, embed_size_per_head)``:
