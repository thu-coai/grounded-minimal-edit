...
class PreTrainedSeq2Seq(nn.Module):
    ...
    def __init__(self, model, tokenizer, smooth_eps, decoder_max_len, criterionSeq):
    ...
    def from_pretrained(
        ...
        model.resize_token_embeddings(len(tokenizer))  # TODO: Authors

        model = cls(model, tokenizer, smooth_eps, decoder_max_len, criterionSeq)
    ...
    def forward(self, input_ids, type_ids, s_len,
        ...
        if self.training:
            ...
            # Decode
            logits, presents = self.model(input_ids=input_ids,
                                          token_type_ids=type_ids)
            shifted_logits = logits[:, :-1].contiguous()

            # Compute loss
            # Seq loss.
            assert not self.criterionSeq.prob
            seq_loss = self.criterionSeq(shifted_logits, target_ids[:, 1:].contiguous(), keep_batch=True)

            if self.smooth_eps > 1e-5:
                # TODO: check
                seq_loss = seq_loss * (1 - self.smooth_eps) - (F.log_softmax(shifted_logits, dim=2).sum(2) *
                                                               target_ids[:, 1:].ne(self.tokenizer.pad_token_id).float()).sum(1) * self.smooth_eps / len(self.tokenizer)
...
class PreTrainedSequenceLabeling(nn.Module):  # TODO: Authors.
    r"""
    ...
    def from_pretrained(
        ...
        model = AutoModel.from_pretrained(
            ...
            transformer_layers=layers,
            pad_id=tokenizer.pad_token_id,
            is_decoder=False,  # TODO: modified by Authors.
        )
